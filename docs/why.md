...
`go-ruby-warden/warden` reimplements Ruby's `warden` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-warden/warden` directly, with no Ruby runtime;
...
Because `warden` is pure Go with cgo disabled, it also compiles to **WebAssembly**
